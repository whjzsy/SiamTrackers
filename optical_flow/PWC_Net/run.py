...
import PIL.Image
import sys
from optical_flow.PWC_Net.flow_visualization.flow_visual import look_flow
try:
	from optical_flow.PWC_Net.correlation import correlation # the custom cost volume layer
except:
...
arguments_strModel = 'chairs-things'
...
class Network(torch.nn.Module):
	def __init__(self):
		...
		self.load_state_dict(torch.load('/home/ubuntu/Desktop/Object_Track/SiamTrackers/optical_flow/PWC_Net/network-' + arguments_strModel + '.pytorch'))
	# end
...
##########################################################
def PWC_flow(pre_image, cur_image, center_pos, size, moduleNetwork):

	# tensor image
	tensorPre = torch.FloatTensor(numpy.array(pre_image).transpose(2, 0, 1).astype(numpy.float32)*(1.0 / 255.0))
	tensorCur = torch.FloatTensor(numpy.array(cur_image).transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))

	optical_flow = estimate(tensorPre, tensorCur, moduleNetwork)
	opticalFlow = numpy.array(optical_flow.numpy().transpose(1, 2, 0), numpy.float32)

	# 取bondingbox內的光流矢量
	sz = pre_image.shape
	# get bbox
	bbox = [int(center_pos[0] - size[0] / 2),
			int(center_pos[1] - size[1] / 2),
			int(size[0]),
			int(size[1])]

	# 提取opticalFlow里面在bbox里的所有点(u,v)
	u = opticalFlow[bbox[1]:bbox[1] + bbox[3], bbox[0]:bbox[0] + bbox[3], 0]
	v = opticalFlow[bbox[1]:bbox[1] + bbox[3], bbox[0]:bbox[0] + bbox[3], 1]

	# 选择bbox中所有点的中指作为目标中心移动的位置
	dx = numpy.median(u)
	dy = numpy.median(v)
	...
	cx = center_pos[0] + dx
	cy = center_pos[1] + dy
	if cx < sz[1] and cy < sz[0] and cx > 0 and cy > 0:
		return cx, cy
	else:
...
if __name__ == '__main__':
	...
	flow = numpy.array(tensorOutput.numpy().transpose(1, 2, 0), numpy.float32)
	look_flow(flow)
