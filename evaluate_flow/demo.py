...
from evaluate_flow.SiamRPNpp import SiamRPNTracker
        ...
        for v_idx, video in enumerate(dataset):
            ...
            for idx, (img, gt_bbox) in enumerate(video):
                ...
                if idx == frame_counter:
                    ...
                    gt_bbox_r = list(map(int, gt_bbox_))
                    cv2.rectangle(img, (gt_bbox_r[0], gt_bbox_r[1]),
                                  (gt_bbox_r[0] + gt_bbox_r[2], gt_bbox_r[1] + gt_bbox_r[3]),
                                  (255, 255, 0), 3)
                elif idx > frame_counter:

                    outputs = tracker.track(img)
                    pred_bbox = outputs['bbox']
                    search_bbox = outputs['search_bbox']

                    search_bbox = list(map(int, search_bbox))
                    cv2.rectangle(img, (search_bbox[0], search_bbox[1]),
                                  (search_bbox[0] + search_bbox[2], search_bbox[1] + search_bbox[3]), (255, 0, 255), 3)
